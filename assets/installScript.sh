...
if [ "$ext" == "ext4" ]; then
	...
	$bb cp /data/local/ext4.ko /data/local/tmp/boot.img-ramdisk/sbin/ext4.ko	
		...
		$bb awk '/mount yaffs2 mtd@cache \/cache nosuid nodev/ && n == 0 { sub(/mount yaffs2 mtd@cache \/cache nosuid nodev/, "exec /sbin/busybox umount /data\n    exec /sbin/busybox umount /system\n    export PHONE_STORAGE /mnt/emmc\n    mount rootfs rootfs / remount\n    exec /sbin/busybox mkdir /mnt/emmc\n    exec /sbin/busybox ln -s /mnt/emmc /emmc\n    mount rootfs rootfs / ro remount\n    devwait '$sdcardblock'\n    exec /sbin/busybox sleep 5\n    exec /sbin/busybox mount '$sdcardblock' /'$storage'\n    exec /sbin/busybox mount '$sdcardreal' /sdcard\n    insmod /'$storage'/BootManager/'$rom'/jbd2.ko\n    insmod /'$storage'/BootManager/'$rom'/ext4.ko\n    chmod 0771 /system\n    exec /sbin/busybox mount -t ext4 -o noatime,nodiratime,commit=19,barrier=0,nobh,nouser_xattr,errors=continue,nodev,rw /'$storage'/BootManager/'"$rom"'/system.img /system\n    exec /system/bin/logwrapper /sbin/e2fsck -p /dev/block/loop0\n    exec /sbin/busybox mount -t ext4 -o noatime,nodiratime,nosuid,nodev,commit=19,barrier=0,nobh,nouser_xattr,errors=continue,rw /'$storage'/BootManager/'"$rom"'/data.img /data\n    exec /system/bin/logwrapper /sbin/e2fsck -p /dev/block/loop1\n    exec /sbin/busybox mount --bind /emmc/BootManager/'$rom'/.android_secure mnt/secure/asec\n    chown system system /data\n    chmod 0771 /data\n    umount \/cache\n    exec /sbin/busybox mount -t ext4 -o noatime,nodiratime,nosuid,nodev,commit=19,barrier=0,nobh,nouser_xattr,errors=continue,rw /'$storage'/BootManager/'"$rom"'/cache.img /cache\n    exec /system/bin/logwrapper /sbin/e2fsck -p /dev/block/loop2\n    chown system cache /cache\n    chmod 0770 /cache"); ++n } { print }' /data/local/tmp/boot.img-ramdisk/init.rc > /data/local/tmp/boot.img-ramdisk/newinit.rc
		else
		$bb awk '/mount yaffs2 mtd@cache \/cache nosuid nodev/ && n == 0 { sub(/mount yaffs2 mtd@cache \/cache nosuid nodev/, "exec /sbin/busybox umount /data\n    exec /sbin/busybox umount /system\n    devwait '$sdcardblock'\n    exec /sbin/busybox sleep 5\n    exec /sbin/busybox mount '$sdcardblock' /'$storage'\n    insmod /'$storage'/BootManager/'$rom'/jbd2.ko\n    insmod /'$storage'/BootManager/'$rom'/ext4.ko\n    chmod 0771 /system\n    exec /sbin/busybox mount -t ext4 -o noatime,nodiratime,commit=19,barrier=0,nobh,nouser_xattr,errors=continue,nodev,rw /'$storage'/BootManager/'"$rom"'/system.img /system\n    exec /system/bin/logwrapper /sbin/e2fsck -p /dev/block/loop0\n    exec /sbin/busybox mount -t ext4 -o noatime,nodiratime,nosuid,nodev,commit=19,barrier=0,nobh,nouser_xattr,errors=continue,rw /'$storage'/BootManager/'"$rom"'/data.img /data\n    exec /system/bin/logwrapper /sbin/e2fsck -p /dev/block/loop1\n    exec /sbin/busybox mount --bind /sdcard/BootManager/'$rom'/.android_secure mnt/secure/asec\n    chown system system /data\n    chmod 0771 /data\n    umount \/cache\n    exec /sbin/busybox mount -t ext4 -o noatime,nodiratime,nosuid,nodev,commit=19,barrier=0,nobh,nouser_xattr,errors=continue,rw /'$storage'/BootManager/'"$rom"'/cache.img /cache\n    exec /system/bin/logwrapper /sbin/e2fsck -p /dev/block/loop2\n    chown system cache /cache\n    chmod 0770 /cache"); ++n } { print }' /data/local/tmp/boot.img-ramdisk/init.rc > /data/local/tmp/boot.img-ramdisk/newinit.rc
		fi
		...
		$bb mv /data/local/tmp/e2fsck /data/local/tmp/boot.img-ramdisk/sbin/e2fsck
		$bb chmod 777 /data/local/tmp/boot.img-ramdisk/sbin/busybox
		$bb chmod 777 /data/local/tmp/boot.img-ramdisk/sbin/e2fsck
	fi
else
	if [ "$board" == "sholes" ]; then
		$bb sed -i 's:mount.*\/system.*: :' /data/local/tmp/boot.img-ramdisk/init.rc
	...
	$bb cp /data/local/tmp/mbcache.ko /data/local/tmp/boot.img-ramdisk/sbin/mbcache.ko
		...
		if [ "$storage" == "emmc" ]; then
		$bb awk '/mount yaffs2 mtd@cache \/cache nosuid nodev/ && n == 0 { sub(/mount yaffs2 mtd@cache \/cache nosuid nodev/, "exec /sbin/busybox umount /data\n    exec /sbin/busybox umount /system\n    export PHONE_STORAGE /mnt/emmc\n    mount rootfs rootfs / remount\n    exec /sbin/busybox mkdir /mnt/emmc\n    exec /sbin/busybox ln -s /mnt/emmc /emmc\n    mount rootfs rootfs / ro remount\n    devwait '$sdcardblock'\n    exec /sbin/busybox sleep 5\n    exec /sbin/busybox mount '$sdcardblock' /'$storage'\n    exec /sbin/busybox mount '$sdcardreal' /sdcard\n    chmod 0771 /system\n    exec /sbin/busybox mount -t ext2 -o rw /'$storage'/BootManager/'"$rom"'/system.img /system\n    exec /system/bin/logwrapper /sbin/e2fsck -p /dev/block/loop0\n    exec /sbin/busybox mount -t ext2 -o rw /'$storage'/BootManager/'"$rom"'/data.img /data\n    exec /system/bin/logwrapper /sbin/e2fsck -p /dev/block/loop1\n    exec /sbin/busybox mount --bind /emmc/BootManager/'$rom'/.android_secure mnt/secure/asec\n    chown system system /data\n    chmod 0771 /data\n    umount \/cache\n    exec /sbin/busybox mount -t ext2 -o rw /'$storage'/BootManager/'"$rom"'/cache.img /cache\n    exec /system/bin/logwrapper /sbin/e2fsck -p /dev/block/loop2\n    chown system cache /cache\n    chmod 0770 /cache"); ++n } { print }' /data/local/tmp/boot.img-ramdisk/init.rc > /data/local/tmp/boot.img-ramdisk/newinit.rc
		else
		$bb awk '/mount yaffs2 mtd@cache \/cache nosuid nodev/ && n == 0 { sub(/mount yaffs2 mtd@cache \/cache nosuid nodev/, "exec /sbin/busybox umount /data\n    exec /sbin/busybox umount /system\n    devwait '$sdcardblock'\n    exec /sbin/busybox sleep 5\n    exec /sbin/busybox mount '$sdcardblock' /'$storage'\n    chmod 0771 /system\n    exec /sbin/busybox mount -t ext2 -o rw /'$storage'/BootManager/'"$rom"'/system.img /system\n    exec /system/bin/logwrapper /sbin/e2fsck -p /dev/block/loop0\n    exec /sbin/busybox mount -t ext2 -o rw /'$storage'/BootManager/'"$rom"'/data.img /data\n    exec /system/bin/logwrapper /sbin/e2fsck -p /dev/block/loop1\n    exec /sbin/busybox mount --bind /sdcard/BootManager/'$rom'/.android_secure mnt/secure/asec\n    chown system system /data\n    chmod 0771 /data\n    umount \/cache\n    exec /sbin/busybox mount -t ext2 -o rw /'$storage'/BootManager/'"$rom"'/cache.img /cache\n    exec /system/bin/logwrapper /sbin/e2fsck -p /dev/block/loop2\n    chown system cache /cache\n    chmod 0770 /cache"); ++n } { print }' /data/local/tmp/boot.img-ramdisk/init.rc > /data/local/tmp/boot.img-ramdisk/newinit.rc
		fi
