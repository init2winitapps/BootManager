...
if [ "$ext" == "ext4" ]; then
	...
	$bb sed -i 's:mount.*\/system.*$:mkdir /mnt 0775 root system\n    mkdir /mnt/sdcard 0000 system system\n    exec /sbin/busybox sleep 10\n    mount vfat '$sdcardblock' /mnt/sdcard\n    chmod 0771 /system\n    exec /sbin/busybox mount -t ext4 -o rw /mnt/sdcard/BootManager/'$rom'/system.img /system:' /data/local/tmp/boot.img-ramdisk/init.$board.rc
	$bb sed -i 's:mount.*\/data.*$:exec /sbin/busybox mount -t ext4 -o rw /mnt/sdcard/BootManager/'$rom'/data.img /data:' /data/local/tmp/boot.img-ramdisk/init.$board.rc
	$bb sed -i 's:mount.*\/cache.*$:exec /sbin/busybox mount -t ext4 -o rw /mnt/sdcard/BootManager/'$rom'/cache.img /cache\n    mkdir /mnt 0775 root system\n    mkdir /mnt/secure 0700 root root\n    mkdir /mnt/secure/asec  0700 root root\n    exec /sbin/busybox mount --bind /mnt/sdcard/BootManager/'$rom'/.android_secure mnt/secure/asec:' /data/local/tmp/boot.img-ramdisk/init.$board.rc
else
	$bb sed -i 's:\#mount.*\/system.*$: :' /data/local/tmp/boot.img-ramdisk/init.$board.rc
	$bb sed -i 's:\#mount.*\/data.*$: :' /data/local/tmp/boot.img-ramdisk/init.$board.rc
	$bb sed -i 's:\#mount.*\/cache.*$: :' /data/local/tmp/boot.img-ramdisk/init.$board.rc
	$bb sed -i 's:mount.*\/system.*$:mkdir /mnt 0775 root system\n    mkdir /mnt/sdcard 0000 system system\n    exec /sbin/busybox sleep 10\n    mount vfat '$sdcardblock' /mnt/sdcard\n    chmod 0771 /system\n    exec /sbin/busybox mount -t ext2 -o rw /mnt/sdcard/BootManager/'$rom'/system.img /system:' /data/local/tmp/boot.img-ramdisk/init.$board.rc
	$bb sed -i 's:mount.*\/data.*$:exec /sbin/busybox mount -t ext2 -o rw /mnt/sdcard/BootManager/'$rom'/data.img /data:' /data/local/tmp/boot.img-ramdisk/init.$board.rc
	$bb sed -i 's:mount.*\/cache.*$:exec /sbin/busybox mount -t ext2 -o rw /mnt/sdcard/BootManager/'$rom'/cache.img /cache\n    mkdir /mnt 0775 root system\n    mkdir /mnt/secure 0700 root root\n    mkdir /mnt/secure/asec  0700 root root\n    exec /sbin/busybox mount --bind /mnt/sdcard/BootManager/'$rom'/.android_secure mnt/secure/asec:' /data/local/tmp/boot.img-ramdisk/init.$board.rc	
fi
